...
const competencyTemplates = [
  ...
  },
  {
    ...
    rubric: competencyStages,
    questions: [
      mc("q1", "What is the primary function of the shear's hold-down?", ["Cools the blade", "Clamps material in place during the cut", "Measures material length", "Controls blade speed"], "B", "Cual es la funcion principal del sujetador de la cizalla?", ["Enfriar la cuchilla", "Sujetar el material durante el corte", "Medir el largo del material", "Controlar la velocidad de la cuchilla"]),
      mc("q2", "What must be checked before starting a shear cycle?", ["Paint color of the machine", "Operator's shift schedule", "Blade condition, clearance/gap setting, and material fit", "Amount of scrap in the bin"], "C", "Que se debe revisar antes de iniciar un ciclo de corte?", ["El color de pintura de la maquina", "El horario del operador", "Condicion de la cuchilla, ajuste de claro/separacion y ajuste del material", "Cantidad de desperdicio en el bote"]),
      mc("q3", "What is the primary pinch/shear point hazard?", ["Back gauge", "Area between the blade and hold-down/table", "Hydraulic unit", "Side frame"], "B", "Cual es el principal peligro de atrapamiento o corte?", ["Tope trasero", "Area entre la cuchilla y el sujetador/mesa", "Unidad hidraulica", "Marco lateral"]),
      mc("q4", "When must both hands be clear of the cutting area?", ["Any time the blade is cycling", "Only during setup", "Only when cutting thick material", "Never - one hand can stay near the blade to guide material"], "A", "Cuando deben estar ambas manos fuera del area de corte?", ["Siempre que la cuchilla este en ciclo", "Solo durante la preparacion", "Solo al cortar material grueso", "Nunca - una mano puede quedarse cerca de la cuchilla para guiar el material"]),
      mc("q5", "What should be done if a guard is missing or damaged?", ["Continue working carefully", "Stop and report immediately; do not operate", "Tape it in place", "Only report at end of shift"], "B", "Que se debe hacer si una guarda falta o esta danada?", ["Seguir trabajando con cuidado", "Detenerse y reportar de inmediato; no operar", "Pegarla con cinta", "Reportarlo solo al final del turno"]),
      mc("q6", "How is the shear locked out for blade changes or jams?", ["Turn the key and walk away", "Unplug at end of day only", "Follow LOTO procedure: de-energize, lock, tag, verify zero energy", "Ask a coworker to hold the switch off"], "C", "Como se bloquea la cizalla para cambios de cuchilla o atorones?", ["Girar la llave y retirarse", "Desconectarla solo al final del dia", "Seguir LOTO: desenergizar, bloquear, etiquetar y verificar energia cero", "Pedir a un companero que sostenga el interruptor apagado"]),
      mc("q7", "What PPE is required at the shear?", ["Sandals and gloves", "No PPE needed for short cuts", "Only safety glasses", "Safety glasses, gloves as appropriate, steel-toe boots, hearing protection if required"], "D", "Que EPP se requiere en la cizalla?", ["Sandalias y guantes", "No se requiere EPP para cortes cortos", "Solo lentes de seguridad", "Lentes de seguridad, guantes segun corresponda, botas con casquillo y proteccion auditiva si se requiere"]),
      yn("q8", "Can the operator identify guards, e-stops, LOTO points, and pinch points without help?", "Yes", "Puede el operador identificar guardas, paros de emergencia, puntos LOTO y puntos de atrapamiento sin ayuda?"),
